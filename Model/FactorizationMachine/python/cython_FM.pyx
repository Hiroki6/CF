...
cdef class cy_FM:
    ...
    def __init__(self,
                    ...
                    np.ndarray[DOUBLE_t, ndim=1, mode="c"] targets):
        self.R = R
    ...
    cdef get_all_error(self):
        ...
        cdef int N = self.N
        cdef np.ndarray[DOUBLE_t, ndim=1, mode="c"] targets = self.targets
    ...
    cdef get_q_error(self, int data_index):
        ...
        cdef np.ndarray[DOUBLE_t, ndim=2, mode="c"] V = self.V
        cdef np.ndarray[DOUBLE_t, ndim=2, mode="c"] R = self.R
        for f xrange(len(V[0]):
            Q[data_index][f] = np.dot(V.T[f], R[data_index])
    ...
    cdef get_error(self, int data_index, int target):
        ...
        cdef np.ndarray[DOUBLE_t, ndim=2, mode="c"] V = self.V
        cdef np.ndarray[DOUBLE_t, ndim=2, mode="c"] R = self.R
        cdef np.ndarray[DOUBLE_t, ndim=1, mode="c"] W = self.W
        cdef np.ndarray[DOUBLE_t, ndim=1, mode="c"] E = self.E

        for f in xrange(len(V[0])):
            iterations += pow(np.dot(V.T[f], R[data_index]), 2) - np.dot(V.T[f]**2, R[data_index]**2)
    ...
    cdef update_weight(self):
        ...
        for l in xrange(n):
            ...
            feature_square_sum = np.sum(R.T[l] ** 2)
    ...
    cdef update_interaction(self):
        ...
        for f in xrange(len(V[0])):
            for l in xrange(n):
                ...
                h_square_sum = 0.0
                for data_index in xrange(N):
                    h_v = (R[data_inde][l] * Q[data_index]) - (pow(R[data_index][l], 2)*V[l][f])
                ...
                for data_index in xrange(N):
                    ...
                    Q[data_index] += (new_v - V[j][f]) * R[data_index][l]
                self.V[l][f] = new_v

        self.E = E
        self.Q = W

    cdef repeat_optimization():
